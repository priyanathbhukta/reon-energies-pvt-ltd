...
import { ArrowRight, ChevronDown, Leaf, Shield, Award, Download, Loader2 } from 'lucide-react'
import html2pdf from 'html2pdf.js'
import BrochurePrintLayout from './brochure/BrochurePrintLayout'
...
const heroImages = [
    '/assets/hero_rooftop_solar.png',
    '/assets/pos_banner.png'
]
...
export default function Hero() {
    ...
    const handleDownloadPDF = async () => {
        ...
        const opt = {
            margin:       0,
            filename:     'REON_ENERGIES_Brochure.pdf',
    ...
    return (
        ...
        >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 lg:gap-16 items-center">
                    ...
                    >
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 flex-wrap">
                            ...
                            <a
                                href={`${import.meta.env.VITE_POS_URL || 'http://localhost:3001'}/register`}
                                target="_blank"
                                rel="noopener noreferrer preconnect dns-prefetch"
                                className="btn-primary text-base px-8 py-4 shadow-emerald hover:scale-105"
                                aria-label="Become a POS"
                            >
                                Become a POS
                                <ArrowRight className="w-5 h-5" />
                            </a>
                            <button
                                ...
                                className="border-2 border-emerald/50 bg-emerald/10 text-white font-semibold px-8 py-4 rounded-full hover:bg-emerald/20 active:scale-95 transition-all duration-200 inline-flex items-center justify-center gap-2 backdrop-blur-sm shadow-emerald hover:shadow-emerald/50 disabled:opacity-75 disabled:hover:scale-100"
                                aria-label="Download Company Brochure"
                            >
                                {isDownloading ? <Loader2 className="w-5 h-5 text-emerald animate-spin" /> : <Download className="w-5 h-5 text-emerald" />}
                                {isDownloading ? 'Generating PDF...' : 'Download Brochure'}
                            </button>
                            <button
                                onClick={() => scrollTo('#services')}
                                className="border-2 border-white/40 text-white font-semibold px-8 py-4 rounded-full hover:bg-white/10 active:scale-95 transition-all duration-200 inline-flex items-center justify-center gap-2 backdrop-blur-sm"
                    ...
                    >
                        <div className="relative">
                            ...
                            <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-white/10">
                                <img
                                    key={currentImageIndex} // Add key to re-trigger animation if desired, or let CSS handle it
                                    src={heroImages[currentImageIndex]}
                                    alt="Solar panels installation on rooftop — REON Energy"
                                    className="w-full h-[400px] lg:h-[500px] object-cover transition-opacity duration-1000 animate-in fade-in"
                                    loading="eager"
                                />
                                {/* Overlay gradient */}
            ...
            {/* Hidden Brochure Layout for PDF Generation */}
            <div style={{ position: 'fixed', top: '200vh', left: 0, zIndex: -50, pointerEvents: 'none' }}>
                <BrochurePrintLayout ref={printRef} />
            </div>
        </section>
    )
}
