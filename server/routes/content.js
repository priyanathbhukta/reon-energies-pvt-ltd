...
// GET /api/content/directors — public
router.get('/directors', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM directors ORDER BY sort_order ASC, id ASC');
    res.json(result.rows);
  } catch (err) {
...
// POST /api/content/directors — admin
router.post('/directors', authMiddleware, async (req, res) => {
  try {
    const { name, role, description, image_url, sort_order } = req.body;
    const result = await pool.query(
      'INSERT INTO directors (name, role, description, image_url, sort_order) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [name, role, description, image_url, sort_order || 0]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
...
// PUT /api/content/directors/:id — admin
router.put('/directors/:id', authMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, role, description, image_url, sort_order } = req.body;
    const result = await pool.query(
      'UPDATE directors SET name=$1, role=$2, description=$3, image_url=$4, sort_order=$5 WHERE id=$6 RETURNING *',
      [name, role, description, image_url, sort_order || 0, id]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: 'Director not found.' });
    res.json(result.rows[0]);
  } catch (err) {
    console.error('Error updating director:', err.message);
    res.status(500).json({ error: 'Failed to update director.' });
  }
});

// DELETE /api/content/directors/:id — admin
router.delete('/directors/:id', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query('DELETE FROM directors WHERE id = $1 RETURNING id', [req.params.id]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Director not found.' });
